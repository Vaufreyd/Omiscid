...
public:
	 ...
	 * @param wait_ms time to wait (no more infinite wait)
	 ...
	 */
	static void Sleep(int nb_milliseconds);

	/** @brief the calling thread sleep
	 * @param nb_microseconds [in] number of microseconds to sleep
	 */
	static void Usleep(int nb_microseconds);
	 ...
	 *
	 */
	static std::thread::id GetCallerThreadId();
...
protected:
	...
	std::unique_ptr<std::thread> MyThread;		/*!< To store created thread a thread */
#else
	std::auto_ptr<std::thread> MyThread;		/*!< To store created thread a thread */
#endif

	/** @brief static method executes in the thread
	 *
	 * call the Run methods of a Thread object.
	 * @param ptr pointer on a Thread object
	 */
	static unsigned int FUNCTION_CALL_TYPE CallRun(void* ptr);
};
