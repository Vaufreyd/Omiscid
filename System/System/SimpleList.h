...
public:
	...
	*/
	...
	/** @brief Function to insert an item after the current element
	* if the CurrentElement if not defined, add tail. subsequent
	* call to Next will give the inserted item
	* @param Val [in] the item to add
	* @return false if a new cell has not been allocated
	*/
	bool Insert( const TYPE& Val );

	/** @brief Function to insert an item before the current element
	* if the CurrentElement if not defined, add head. subsequent
	* call to Next will provide the item after the CurrentElement
	* @param Val [in] the item to add
	* @return false if a new cell has not been allocated
	*/
	bool InsertBefore( const TYPE& Val );
...
// Access fonction
template <typename TYPE>
bool SimpleList<TYPE>::Insert( const TYPE& Val )
{
	...
	// Validate new element and insert it after the CurrentElement
	tmp->ElementContainer = Val;
	tmp->NextElement = CurrentElement->NextElement;
	CurrentElement->NextElement = tmp;

	// PreviousElement and CurrentElement are still good
...
// Access fonction
template <typename TYPE>
bool SimpleList<TYPE>::InsertBefore( const TYPE& Val )
{
	// Ok, we do not have any other element
	...
	// Validate new element and insert it after the CurrentElement
	tmp->ElementContainer = Val;
	tmp->NextElement = CurrentElement;
	PreviousElement->NextElement = tmp;

	// Make next work !
	PreviousElement = tmp;
	// CurrentElement is still ok
