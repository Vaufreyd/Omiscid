...
namespace Omiscid {
	...
	// Network API require an int on Windows
	#define CONVERT_SIZE_FOR_RECVSEND int
#else
	// size_t on other system
	#define CONVERT_SIZE_FOR_RECVSEND size_t
#endif
...
public:
  ...
  enum { DEFAULT_CONNECTION_TIMEOUT = 1000 };	// 1 second

  /**
   * @brief Initiate a connection on a socket
   * @param addr [in] address where connect
   * @param port [in] port where connect
   * @return ture if the connection was done
   */
  bool Connect(const SimpleString addr, int port, int ConnectionTimeout = DEFAULT_CONNECTION_TIMEOUT );
   ...
   */
  int Recv(size_t len, unsigned char* buf, struct sockaddr_in* pfrom = NULL);
   ...
   */
  int Send(size_t len, const char* buf);
   ...
   * @exception raise SocketException if error during sending
   */
  int SendTo(size_t len, const char* buf, struct sockaddr_in* dest );
   ...
   */
  bool SetBlockingMode( bool BlockingMode );
